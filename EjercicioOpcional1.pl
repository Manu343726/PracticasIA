...
%Estado Inicial del problema: estado(1,1,2,2,1,1,1)
%Estado Objetivo del problema: estado(0,0,0,0,0,0,0)
...
movimiento(estado(P,M,Ho,Ha,Pol,Lad,1),estado(P,M,Ho,Ha,NPol,NLad,0)):-
	Pol>0,Lad>0,NPol is Pol-1,NLad is Lad-1,not(peligro(estado(P,M,Ho,Ha,NPol,NLad,0))).

%Mueve al ladrón y al policia del lado derecho al izquierdo.
movimiento(estado(P,M,Ho,Ha,Pol,Lad,0),estado(P,M,Ho,Ha,NPol,NLad,1)):-
	Pol<1,Lad<1,NPol is Pol+1,NLad is Lad+1,not(peligro(estado(P,M,Ho,Ha,NPol,NLad,1))).

%Mueve al policia del lado izquierdo al derecho.
movimiento(estado(P,M,Ho,Ha,Pol,Lad,1),estado(P,M,Ho,Ha,NPol,Lad,0)):-
	Pol>0,NPol is Pol-1,not(peligro(estado(P,M,Ho,Ha,NPol,Lad,0))).

%Mueve al policia del lado derecho al izquierdo.
movimiento(estado(P,M,Ho,Ha,Pol,Lad,0),estado(P,M,Ho,Ha,NPol,Lad,1)):-
	Pol<1,NPol is Pol+1,not(peligro(estado(P,M,Ho,Ha,NPol,Lad,1))).

%Mueve al policia y a un niño de izquierda a derecha
movimiento(estado(P,M,Ho,Ha,Pol,Lad,1),estado(P,M,NHo,Ha,NPol,Lad,0)):-
	Pol>0,Ho>0,NPol is Pol-1,NHo is Ho-1,not(peligro(estado(P,M,NHo,Ha,NPol,Lad,0))).

%Mueve al policia y a un niño de derecha a izquierda 
movimiento(estado(P,M,Ho,Ha,Pol,Lad,0),estado(P,M,NHo,Ha,NPol,Lad,1)):-
	Pol<1,Ho<2,NPol is Pol+1,NHo is Ho+1,not(peligro(estado(P,M,NHo,Ha,NPol,Lad,1))).

%Mueve al policia y a una niña de izquierda a derecha
movimiento(estado(P,M,Ho,Ha,Pol,Lad,1),estado(P,M,Ho,NHa,NPol,Lad,0)):-
	Pol>0,Ha>0,NPol is Pol-1,NHa is Ha-1,not(peligro(estado(P,M,Ho,NHa,NPol,Lad,0))).

%mueve al policia y a una niña de derecha a izquierda
movimiento(estado(P,M,Ho,Ha,Pol,Lad,0),estado(P,M,Ho,NHa,NPol,Lad,1)):-
	Pol<1,Ha<2,NPol is Pol+1,NHa is Ha+1,not(peligro(estado(P,M,Ho,NHa,NPol,Lad,1))).

%Mueve al padre de izquierda a derecha
movimiento(estado(P,M,Ho,Ha,Pol,Lad,1),estado(NP,M,Ho,Ha,Pol,Lad,0)):-
	P>0,NP is P-1,not(peligro(estado(NP,M,Ho,Ha,Pol,Lad,0))).

%Mueve al padre de derecha a izquierda
movimiento(estado(P,M,Ho,Ha,Pol,Lad,0),estado(NP,M,Ho,Ha,Pol,Lad,1)):-
	P<1,NP is P+1,not(peligro(estado(NP,M,Ho,Ha,Pol,Lad,1))).

%Mueve a la madre de izquierda a derecha
movimiento(estado(P,M,Ho,Ha,Pol,Lad,1),estado(P,NM,Ho,Ha,Pol,Lad,0)):-
	M>0,NM is M-1,not(peligro(estado(P,NM,Ho,Ha,Pol,Lad,0))).

%Mueve a la madre de derecha a izquierda
movimiento(estado(P,M,Ho,Ha,Pol,Lad,0),estado(P,NM,Ho,Ha,Pol,Lad,1)):-
	M<1,NM is M+1,not(peligro(estado(P,NM,Ho,Ha,Pol,Lad,1))).

%Mueve al padre y a un hijo de izquierda a derecha
movimiento(estado(P,M,Ho,Ha,Pol,Lad,1),estado(NP,M,NHo,Ha,Pol,Lad,0)):-
	P>0,Ho>0,NP is P-1,NHo is Ho-1,not(peligro(estado(NP,M,NHo,Ha,Pol,Lad,0))).

%Mueve al padre y a un hijo de derecha a izquierda
movimiento(estado(P,M,Ho,Ha,Pol,Lad,0),estado(NP,M,NHo,Ha,Pol,Lad,1)):-
	P<1,Ho<2,NP is P+1,NHo is Ho+1,not(peligro(estado(NP,M,NHo,Ha,Pol,Lad,1))).

%Mueve a la madre y a una hija de izquierda a derecha
movimiento(estado(P,M,Ho,Ha,Pol,Lad,1),estado(P,NM,Ho,NHa,Pol,Lad,0)):-
	M>0,Ha>0,NM is M-1,NHa is Ha-1,not(peligro(estado(P,NM,Ho,NHa,Pol,Lad,0))).

%Mueve a la madre y a una hija de derecha a izquierda
movimiento(estado(P,M,Ho,Ha,Pol,Lad,0),estado(P,NM,Ho,NHa,Pol,Lad,1)):-
	M<1,Ha<2,NM is M+1,NHa is Ha+1,not(peligro(estado(P,NM,Ho,NHa,Pol,Lad,1))).

%Mueve al padre y a la madre de izquierda a derecha
movimiento(estado(P,M,Ho,Ha,Pol,Lad,1),estado(NP,NM,Ho,Ha,Pol,Lad,0)):-
	P>0,M>0,NP is P-1, NM is M-1,not(peligro(estado(NP,NM,Ho,Ha,Pol,Lad,0))).

%Mueve al padre y a la madre de derecha a izquierda
movimiento(estado(P,M,Ho,Ha,Pol,Lad,0)),estado(NP,NM,Ho,Ha,Pol,Lad,1)):-
P<1,M<1,NP is P+1,NM is M+1,not(peligro(estado(NP,NM,Ho,Ha,Pol,Lad,1))).
...
%---------------------------------------------------------------------
%--------------------LLamada para solucionar--------------------------
%---------------------------------------------------------------------

soluciona:-camino(estado(1,1,2,2,1,1,1),estado(0,0,0,0,0,0,0),estado(1,1,2,2,1,1,1),_).